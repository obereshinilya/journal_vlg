...
class XMLController extends Controller
{
    ...
    public function hand_for_masdu($hours_xml, $date_xml, $hour)
    {
        if ($hour && $hour != 0) {
            $str = strtotime($date_xml . '+' . $hour . 'hours ');
            $timestamp = date('Y-m-d H:i:s', $str);     //время отправки местное
        } else {
            $timestamp = date($date_xml);
        }
...
//                $data_in_journal['option'] = 'Отсутствие связи с sftp-сервером!';
            ...
            $contents = $contents . "     <ReferenceTime time=\"" . date('Y-m-d') . "T" . date('10:00:00') . "+03:00\"/>\n";
        ...
        try {
            try {
                ...
                $disk = Storage::build([
                    ...
                    'visibility' => 'public',
                    'permPublic' => 0777, /// <- this one did the trick
                    'root' => $setting_sftp['path_sftp'].'log/',
                ]);
