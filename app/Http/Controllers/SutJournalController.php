...
class SutJournalController extends Controller
{
    ...
    {   //сохранение только отдельных строк
    ...
    public function print_journal_smeny_period($date_start, $date_end)
    {
        $period = new \DatePeriod(
            new \DateTime($date_start),
            new \DateInterval('P1D'),
            new \DateTime($date_end . '23:59')
        );
        foreach ($period as $day)
            $dates[] = $day->format('Y-m-d');
        return view('web.pdf_form.pdf_journal_smeny', compact('dates'));
    }
