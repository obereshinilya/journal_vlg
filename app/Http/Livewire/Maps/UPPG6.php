...
class UPPG6 extends Component
{
    ...
    public function render()
    {
        $j=0;
        for ($i = 9; $i <= 17; $i++) {
            $this_calc_tb = Ref_obj::find(699)->elem_to_calc_tb->where('from_oto', '=', $i)->first();
            $this->colors[$j] = $this->colors($this_calc_tb->op_tb);
            $j++;
        }


        return view('livewire.maps.u-p-p-g6',['opo' =>Ref_opo::find(7)]);
    }
}
