...
@section('content')
    ...
    <div style="width: 100%; display: inline-flex">
        ...
        <button  id="add_record" onclick="add_record(); this.style.display = 'none'" class="button button1" style="float: left; margin-top: 1%">Добавить ППР</button>
           ...
           $.ajax({
               ...
               success: function (res) {
                   ...
                   for (var row of res){
                       ...
                       if (!row['comment'])
                           row['comment'] = '-'
                       tr.innerHTML += `<td style="text-align: center" class="id">${lenght}</td>`
                       tr.innerHTML += `<td style="text-align: center" class="object">${row['object']}</td>`
                       tr.innerHTML += `<td style="background: white; text-align: center"><input disabled class="plan_begin" style="border: none; width: 95%; text-align: center" type="date" value="${row['plan_begin'].split(' ')[0]}" /><br><input disabled class="plan_begin" style="border: none; width: 95%; text-align: center; margin-top: 3px" type="time" value="${row['plan_begin'].split(' ')[1]}" /></td>`
                       tr.innerHTML += `<td style="background: white; text-align: center"><input disabled class="plan_end" style="border: none; width: 95%; text-align: center" type="date" value="${row['plan_end'].split(' ')[0]}" /><br><input disabled class="plan_end" style="border: none; width: 95%; text-align: center; margin-top: 3px" type="time" value="${row['plan_end'].split(' ')[1]}" /></td>`
                       if (row['fact_begin']){
                           tr.innerHTML += `<td style="background: white; text-align: center"><input disabled class="fact_begin" style="border: none; width: 95%; text-align: center" type="date" value="${row['fact_begin'].split(' ')[0]}" /><br><input disabled class="fact_begin" style="border: none; width: 95%; text-align: center; margin-top: 3px" type="time" value="${row['fact_begin'].split(' ')[1]}" /></td>`
                       } else {
                           tr.innerHTML += `<td style="background: white; text-align: center"><input onchange="change_fact_begin(this, 'date')" class="fact_begin" style="border: none; width: 95%; text-align: center" type="date" /><br><input onchange="change_fact_begin(this, 'time')" class="fact_begin" style="border: none; width: 95%; text-align: center; margin-top: 3px" type="time"/></td>`
                       }
                       if (row['fact_end']){
                           tr.innerHTML += `<td style="background: white; text-align: center"><input disabled class="fact_end" style="border: none; width: 95%; text-align: center" type="date" value="${row['fact_end'].split(' ')[0]}" /><br><input disabled class="fact_end" style="border: none; width: 95%; text-align: center; margin-top: 3px" type="time" value="${row['fact_end'].split(' ')[1]}" /></td>`
                       }else {
                           tr.innerHTML += `<td style="background: white; text-align: center"><input disabled onchange="change_fact_end(this)" class="fact_end" style="border: none; width: 95%; text-align: center" type="date"/><br><input disabled onchange="change_fact_end(this)" class="fact_end" style="border: none; width: 95%; text-align: center; margin-top: 3px" type="time" /></td>`
                       }
                       tr.innerHTML += `<td onblur="update_record(this.parentNode)" style="background: white; text-align: center" contenteditable="true" class="type_job">${row['type_job']}</td>`
                       tr.innerHTML += `<td onblur="update_record(this.parentNode)" style="background: white; text-align: center" contenteditable="true" class="comment">${row['comment']}</td>`
