...
<p id="mesto" style="display: none">{{$mesto}}</p>
    ...
    <div style="display: inline-flex; width: 100%">
            <h3 style="text-align: center; width: 100%">Балансовый отчет за {{$date}} год</h3>
    </div>
    <style>
        .choice-period-btn {
            display: none;
                ...
                success: function (res) {
                    var type = document.getElementById('mesto').textContent
                        ...
                        for (var j=1; j<=Object.keys(res[type]).length; j++){
                            ...
                            if (res['plan'][type][j]){
                                td_plan.innerHTML = `<span>${res['plan'][type][j]}</span>`
                                last_plan = Number(res['plan'][type][j])
                                buff_plan += Number(res['plan'][type][j])
                            }else {
                                td_plan.innerHTML = `<span>0.000</span>`
                            }

                            var td_otkl=document.createElement('td')
                            td_otkl.innerHTML = `<span>${(last_tovar-last_plan-last_out).toFixed(3)}</span>`
