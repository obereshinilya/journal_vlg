                ...
                success:(res)=>{
                    ...
                    for (var i = 0; i<res.length; i++){
                        var tr=document.createElement('tr')
                            tr.innerHTML+=`<td><span data-type="text" style="text-align: center">${res[i]['create'].split('.')[0]}</span></td>`
                            tr.innerHTML+=`<td><span data-type="text" style="text-align: center">${res[i]['dz']}</span></td>`
                            tr.innerHTML+=`<td><span data-type="text" style="text-align: center">${res[i]['autor']}</span></td>`
                            tr.innerHTML+=`<td><span data-type="text" style="text-align: center" contenteditable="true" onblur="save_comment(${res[i]['id']}, this.textContent)">${res[i]['comment']}</span></td>`
                        if (res[i]['check']){
                            tr.innerHTML+=`<td><span data-type="text" style="text-align: center">${res[i]['info']}</span></td>`
                        }else {
                            tr.innerHTML+=`<td style="text-align: center"><button class="button button1" onclick="confirm_dz(${res[i]['id']})">Принять задание</button></td>`
                        }
